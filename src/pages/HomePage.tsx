      ...
      {events.length > 0 && (
        <section className="py-16 px-6 md:px-12 bg-gradient-to-br from-nasmed-navy via-[#0d2558] to-nasmed-mid-blue">
          <div className="max-w-[1280px] mx-auto">
            ...
            <div className="flex items-center justify-between mb-10">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-[11px] font-bold tracking-[2px] uppercase text-nasmed-green-light">Events</span>
                </div>
                <h2 className="font-heading text-white font-bold leading-tight" style={{ fontSize: 'clamp(24px, 3vw, 36px)' }}>
                  Upcoming Events
                </h2>
              </div>
              <Link to="/news#featured-event" className="text-nasmed-green-light text-[13px] font-semibold hover:text-white transition-colors hidden md:block">
                View All &amp; Register →
              </Link>
            </div>

            {/* Featured event — first event, big card */}
            {(() => {
              const ev = events[0];
              return (
                <div className="bg-white/[0.07] border border-white/15 rounded-2xl p-7 md:p-9 mb-5">
                  <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-8 items-start">
                    <div>
                      <div className="flex gap-2 flex-wrap mb-4">
                        ...
                        <span className="bg-white/10 border border-white/20 text-white/70 py-1 px-3 rounded-full text-[11px] font-semibold tracking-wide">
                          🌐 {ev.location}
                        </span>
                      </div>
                      <h3 className="font-heading text-white font-bold leading-tight mb-3" style={{ fontSize: 'clamp(20px, 2.5vw, 30px)' }}>
                        {ev.title}
                      </h3>
                      <p className="text-white/65 text-[14px] leading-relaxed max-w-[580px] mb-5">
                        {ev.description}
                      </p>
                      <div className="flex flex-wrap gap-4 text-white/55 text-[13px] mb-6">
                        {ev.event_date && (
                          <span>📅 {new Date(ev.event_date).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}</span>
                        )}
                        <span>📍 {ev.location}</span>
                      </div>
                      <div className="flex gap-3 flex-wrap">
                        <Link to="/news#featured-event" className="btn-primary">Register Now →</Link>
                        <Link to="/news#featured-event" className="btn-outline-hero">View Full Details</Link>
                      </div>
                    </div>
                    <div className="flex flex-row lg:flex-col gap-3 lg:min-w-[150px]">
                      <div className="bg-white/10 backdrop-blur border border-white/20 rounded-2xl p-5 text-center flex-1 lg:flex-none">
                        ...
                        <div className="font-heading text-white font-bold text-[40px] leading-none">{ev.day_label}</div>
                        <div className="font-heading text-nasmed-green-light text-[17px] font-bold tracking-wide">{ev.month_label}</div>
                        <div className="text-white/40 text-[12px] mt-0.5">{ev.event_date ? new Date(ev.event_date).getFullYear() : ""}</div>
                      </div>
                      ...
                      {ev.registration_fee > 0 && (
                        <div className="bg-white/8 border border-white/15 rounded-xl p-4 text-center flex-1 lg:flex-none">
                          <div className="font-heading text-white text-[20px] font-bold">₦{ev.registration_fee.toLocaleString()}</div>
            ...
            {/* Remaining events — compact list cards */}
            {events.length > 1 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {events.slice(1).map(ev => (
                  <div key={ev.id} className="bg-white/[0.05] border border-white/10 rounded-xl p-5 flex gap-4 items-start hover:bg-white/[0.09] transition-colors">
                    <div className="flex-shrink-0 bg-white/10 border border-white/15 rounded-xl p-3 text-center min-w-[58px]">
                      <div className="font-heading text-white font-bold text-[26px] leading-none">{ev.day_label}</div>
                      <div className="text-nasmed-green-light text-[10px] font-bold tracking-wide mt-0.5">{ev.month_label}</div>
                      <div className="text-white/35 text-[10px]">{ev.event_date ? new Date(ev.event_date).getFullYear() : ""}</div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-heading text-white text-[15px] font-bold leading-snug mb-1">{ev.title}</h4>
                      <p className="text-white/55 text-[12px] mb-2">📍 {ev.location}</p>
                      <div className="flex gap-3 items-center flex-wrap">
                        <span className="text-nasmed-green-light text-[12px] font-semibold">FREE for members</span>
                        {ev.registration_fee > 0 && (
                          <span className="text-white/45 text-[12px]">· ₦{ev.registration_fee.toLocaleString()} others</span>
                        )}
                        <Link to="/news#featured-event" className="text-[12px] font-semibold text-nasmed-green-light border border-nasmed-green/40 rounded-lg py-1 px-3 hover:bg-nasmed-green/20 transition-colors">
                          Register →
                        </Link>
                      </div>
                    </div>
            ...
            <div className="mt-6 text-center md:hidden">
              <Link to="/news#featured-event" className="text-nasmed-green-light text-[13px] font-semibold">
